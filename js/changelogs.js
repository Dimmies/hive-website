...
let Logs = { // NEWEST LOG SHOULD BE ON TOP || MAKE SURE TO EDIT THE CURRENT VAR
	...
	"Thursday, April 28th 2022": {
		...
		"logs": {
			...
			2: { type: "add", desc: "Added sounds when opening PD/EMS Lockers" },
			3: { type: "bug", desc: "Fixed Boosting not resetting if you disconnect" },
			4: { type: "add", desc: "Added Boost Vehicle description to police alerts" },
			5: { type: "change", desc: "Boost Reputation now resets every 30 days" },
			6: { type: "change", desc: "Cocaine Dealer now only works during certain times" },
			7: { type: "optimize", desc: "Optimized Cocaine" },
			8: { type: "change", desc: "Changed how to obtain PSB Requirement" },
			9: { type: "add", desc: "Completely rewrote house robberies" },
			10: { type: "change", desc: "Limited Humane defib uses" },
			11: { type: "bug", desc: "Fixed Meth exploit" },
			12: { type: "add", desc: "Completely rewrote PSB Heist" },
			13: { type: "add", desc: "Added new PSB Interior" },
			14: { type: "add", desc: "Completely Rewrote Bank Truck Robberies" },
			15: { type: "change", desc: "Increased amount of water and fertilizer that is added to Weed Plants" },
			16: { type: "optimize", desc: "Optimized Graffiti" },
			17: { type: "add", desc: "Added Graffiti from Mawi" },
			18: { type: "add", desc: "Added 2 more Ballas Graffiti" },
			19: { type: "add", desc: "Added Cat Cafe Graffiti (@Anj)" },
			20: { type: "add", desc: "Added Best Buds Graffiti (@Anj)" },
			21: { type: "add", desc: "Added Raccoon Sheriff Graffiti (@Anj)" },
			22: { type: "add", desc: "Added Doppler Cinema where you can watch YouTube Videos, Twitch Streams and listen to Soundcloud together" },
			23: { type: "bug", desc: "Fixed football not deleting sometimes" },
			24: { type: "bug", desc: "Fixed Spin to Win wheel having weird text in the middle" },
			25: { type: "add", desc: "Added a teleport to make parachuting easier" },
			26: { type: "add", desc: "Added /unbindemote command help" },
			27: { type: "bug", desc: "Fixed /unbindemote not automatically refreshing" },
			28: { type: "add", desc: "Made Sandy Sheriff Station functional" },
			29: { type: "add", desc: "Added Crafting Professions, this makes it so you have to have a crafting profession to be able to craft certain items" },
			30: { type: "change", desc: "Changed how to obtain Laptop Pro" },
			31: { type: "bug", desc: "Fixed Fake Plates sticking on vehicles when you store them in garages" },
			32: { type: "bug", desc: "Fixed Inventories not working when you apply a fake plate to a vehicle" },
			33: { type: "change", desc: "Changed progress bar color" },
			34: { type: "change", desc: "Changed third eye color" },
			35: { type: "add", desc: "Added circles to centers of third eye zones to make them easier to find" },
			36: { type: "optimize", desc: "Optimized Third Eye" },
			37: { type: "bug", desc: "Fixed ghost weapons being stuck in hand when being revived" },
			38: { type: "add", desc: "Made large melee weapons show on players backs (bat, golfclub, and crowbar)" },
			...
			46: { type: "add", desc: "Added splints that heal injured legs and stops stumbling" },
			47: { type: "add", desc: "Added 2 missing ammunations" },
			48: { type: "add", desc: "Added new Ammunation interiors" },
			49: { type: "optimize", desc: "Optimized Vehicle Health" },
			50: { type: "change", desc: "Limited vehicles trunks to 2 people" },
			51: { type: "optimize", desc: "Optimized Vehicle Trunks" },
			52: { type: "add", desc: "Added minigame for hotwiring vehicles" },
			53: { type: "optimize", desc: "Optimized vehicle hotwiring" },
			54: { type: "bug", desc: "Fixed Hunting Rifle not having Weapon Damage" },
			55: { type: "bug", desc: "Fixed error when shooting Hunting Rifle" },
			56: { type: "change", desc: "Decreased frequency of stumbling when you have a leg injury" },
			57: { type: "change", desc: "Decreased Staggering chance when hit in certain areas" },
			58: { type: "add", desc: "Added disabling stagger chance if you get hit in the spine or upper body and you have a bullet proof vest on" },
			59: { type: "add", desc: "Added Bee Keeping" },
			60: { type: "bug", desc: "Fixed Mintys Business" },
			61: { type: "add", desc: "Added missing items from Pops Diner" },
			62: { type: "add", desc: "Added ability to sell vehicles back to PDM for 50%" },
			63: { type: "add", desc: "Added Sanctus, Hakuchou Drag, Hearse and Taco Truck to PDM" },
			64: { type: "add", desc: "Added Electrician Job" },
			65: { type: "bug", desc: "Fixed seeds sometimes not growing to 100%" },
			66: { type: "bug", desc: "Fixed seed outline drawing when it shouldn't" },
			67: { type: "optimize", desc: "Optimized Garbage Job" },
			68: { type: "add", desc: "Added House Robbery Reputation" },
			69: { type: "add", desc: "Added Mechanic count to Police MDT" },
			70: { type: "add", desc: "Added ability for LEO High Command to Give/Remove certifications to give people access to certain vehicles in the garage depending on their certs" },
			71: { type: "add", desc: "Added Evidence lab to Davis PD" },
			72: { type: "add", desc: "Added Scrap Job" },
			73: { type: "add", desc: "Added Cinema MLO" },
			74: { type: "add", desc: "Added Bean Machine MLO" },
			75: { type: "optimize", desc: "Optimized NPC Dialogue" },
			76: { type: "change", desc: "Moved Percocets Seller" },
			77: { type: "change", desc: "Moved Cocaine Chem Guy" },
			78: { type: "change", desc: "Moved Cocaine Dealer Guy" },
			79: { type: "add", desc: "Added Crafting Professions app to Laptop Pro" },
			80: { type: "add", desc: "Added ability to set a custom wallpaper for Laptops" },
			...
			88: { type: "change", desc: "Increased PD & EMS Pay" },
			89: { type: "change", desc: "Decreased Vespucci Markets Rental Prices" },
			90: { type: "change", desc: "Decreased crafting requirements for all ammo" },
			91: { type: "add", desc: "Added new Paleto Bank MLO" },
			92: { type: "change", desc: "Increased sell price of Diamonds" },
			93: { type: "bug", desc: "Fixed EMS Maverick 4th not being working (@Ice)" },
			94: { type: "change", desc: "Decreased Cocaine Payout" },
			95: { type: "change", desc: "Made Zipties Stackable" },
			96: { type: "bug", desc: "Another attempt at fixing Bennys resetting fuel" },
			97: { type: "bug", desc: "Fixed Cow Slaughtering progress bar saying Feeding" },
			98: { type: "bug", desc: "Fixed roulette tables not working" },
			99: { type: "bug", desc: "Fixed being stuck in place when harvesting/destroying weed plants" },
		},
	},
	...
	"Monday, March 21st 2022": {
		...
		"logs": {
			...
			11: { type: "add", desc: "Added new Scout for EMS (@zeuz)" },
			12: { type: "add", desc: "Added new Boat for EMS (@zeuz)" },
			13: { type: "add", desc: "Added new Buffalo STX for Police (@zeuz)" },
			14: { type: "change", desc: "Made Laptop UI larger" },
			15: { type: "add", desc: "Added locks to Pillbox garage doors" },
			16: { type: "optimize", desc: "Optimized networked vehicle spawning" },
		},
	},
	...
	"Friday, February 11th 2022": {
		...
		"logs": {
			...
			2: { type: "add", desc: "Added ~30 new vehicles to PDM" },
			3: { type: "add", desc: "Added roughly 600 new wheels" },
			4: { type: "add", desc: "Added Laptop System (Currently only has LSPT and Hayes apps, but will be expanded to include a whole lot more 0.0)" },
			5: { type: "add", desc: "Added Hunger and Thirst warning sounds" },
			6: { type: "add", desc: "Added seatbelt reminder alarm" },
			7: { type: "add", desc: "Added duty cacheing (if you log off, your duty status will stay for 10 minutes before taking you off duty.)" },
			8: { type: "add", desc: "Added new LSSD Ranks" },
			9: { type: "change", desc: "Increased DOJ Paychecks" },
			10: { type: "bug", desc: "Fixed Cocaine Drug Handoff showing incorrect location" },
			11: { type: "add", desc: "Added a Criminal Doctor" },
			12: { type: "change", desc: "When you break your hacker laptop, it now gives you Electronics" },
			13: { type: "change", desc: "Waypoint now clears when detaching from a call" },
			14: { type: "bug", desc: "Fixed weapon being holstered when cancelling an animation" },
			15: { type: "add", desc: "Added ability for police to raid houses and apartments" },
			16: { type: "bug", desc: "Fixed transferring money to someone" },
			17: { type: "bug", desc: "Fixed not being able to apply stock wheels" },
			18: { type: "bug", desc: "Fixed decals following outfits" },
			19: { type: "add", desc: "Added boat garage to Cayo Perico" },
			20: { type: "change", desc: "Rewrote Mining" },
			21: { type: "add", desc: "Added Lumber Job" },
			22: { type: "bug", desc: "Fixed 'Rob Money' button sticking sometimes" },
			23: { type: "remove", desc: "Removed old 'IBAN' system on Phone" },
			24: { type: "add", desc: "Added sharing anonymous contact details" },
			25: { type: "add", desc: "Added Notes app to phone" },
			26: { type: "add", desc: "Added ability to pat down someone" },
			27: { type: "add", desc: "Added watch to Hardware store" },
			28: { type: "add", desc: "Added /gkn command to give keys to the nearest player" },
			29: { type: "bug", desc: "Fixed weapon ammo sometimes going negative" },
			30: { type: "add", desc: "Added Flashlight and Diving Gear to EMS store" },
			31: { type: "change", desc: "Tweaked vehicle damage" },
			32: { type: "add", desc: "Added name to Police MDT" },
			33: { type: "add", desc: "Added ability to Locate Player house to Police MDT" },
			34: { type: "add", desc: "Added ability to expunge a record to Police MDT" },
			35: { type: "add", desc: "Added panic button sounds" },
			36: { type: "change", desc: "Tweaked placing props for Police" },
			37: { type: "change", desc: "Police prop menu reopens when you finish placing" },
			38: { type: "change", desc: "Updated a ton of maps" },
			39: { type: "change", desc: "Slightly increased default /me font size" },
			40: { type: "bug", desc: "Fixed Welcome Gift spelling error" },
			41: { type: "bug", desc: "Fixed Cigarettes and Cigars" },
			42: { type: "add", desc: "Added New Camera (/cam), Boom Mic (/bmic) and Handheld Mic (/mic)" },
			43: { type: "add", desc: "Added locks to Courthouse" },
			44: { type: "bug", desc: "Fixed sometimes not being able to enter apartment" },
			45: { type: "optimize", desc: "Optimized Bennys" },
			46: { type: "add", desc: "Added SMG showing on back" },
			47: { type: "add", desc: "Added Roxanne livery (@Pawbles)" },
			48: { type: "change", desc: "Changed Hellfire engine sound (@Zeuz)" },
			49: { type: "change", desc: "Cleaned up and optimized a few resources" },
		},
	},
	...
	"Wednesday, January 26th 2022": {
		...
		"logs": {
			...
			4: { type: "change", desc: "Rewrote jump tripping, now goes based off of how many times you jump rather than completely random chance" },
	...
	"Thursday, January 13th 2022": {
		...
		"logs": {
			...
			2: { type: "add", desc: "Added parent inheritance sliders to character creator" },
			3: { type: "add", desc: "Added more error handling to Coke Chemical delivery task" },
			4: { type: "add", desc: "Added arm wrestling" },
			5: { type: "add", desc: "Added full support for player owned businesses" },
			6: { type: "add", desc: "Added ability to freeze bank accounts" },
			7: { type: "change", desc: "Clothing Stores and Barber shops now use zones instead of markers" },
			8: { type: "add", desc: "Added outfit locker to clothing stores" },
			9: { type: "bug", desc: "Fixed female /shoes command not working correctly" },
			10: { type: "add", desc: "Added crafting for Rifle and Shotgun ammo" },
			11: { type: "add", desc: "Added crafting for lockpicks" },
			12: { type: "remove", desc: "Removed a flatscreen TV object that broke house decoration" },
			13: { type: "add", desc: "Added automatically turning on/off watch depending on if you have one" },
			14: { type: "add", desc: "Added ability to rob someones money" },
			15: { type: "change", desc: "Completely rewrote weapon ammo saving/loading (Ammo now save on the gun item rather than gloablly. Yes, you'll lose your current ammo)" },
			16: { type: "bug", desc: "Fixed some issues with Advertisements" },
			17: { type: "add", desc: "Added ability to view all bank accounts in bank phone app" },
			18: { type: "change", desc: "Decreased police SMG recoil" },
			19: { type: "add", desc: "Added some boss menu changes to support player owned businesses" },
			20: { type: "bug", desc: "Fixed boss menu hiring not updating immediately" },
			21: { type: "change", desc: "Rewrote paychecks" },
			22: { type: "add", desc: "Added template buttons in police MDT to auto populate report descriptions with a template" },
			23: { type: "add", desc: "Added sitting animation when hard cuffed" },
			24: { type: "add", desc: "Added progress bar to car wash" },
			25: { type: "bug", desc: "Fixed some issues with casino chips" },
			26: { type: "add", desc: "Added stackable /me" },
			27: { type: "bug", desc: "Fixed armor not saving when using meth" },
			28: { type: "add", desc: "Added tripping when jumping" },
			29: { type: "bug", desc: "Fixed paying invoices" },
			30: { type: "add", desc: "Added PD Duty Belts (@mzry)" },
			31: { type: "add", desc: "Added Female/Male pocket bandana colors (@mzry)" },
			32: { type: "remove", desc: "Removed Female Hiking Backpack (@mzry)" },
			33: { type: "bug", desc: "Fix Sentinel Classic suspension (@Klutch)" },
			34: { type: "bug", desc: "Fix Sentinel gauge color (@Klutch)" },
			35: { type: "bug", desc: "Possibly fixed Roxanne Horn (@Klutch)" },
			36: { type: "bug", desc: "Fixed EMS Chopper Doors (@Klutch)" },
			37: { type: "change", desc: "Changed PD and EMS helicopter sounds (@zeuz)" },
		},
	},
	...
	"Wednesday, December 15th 2021": {
		...
		"logs": {
			...
			2: { type: "add", desc: "Added /shirt, /pants, /shoes, /bag and /hair to toggle pieces of clothing" },
			3: { type: "bug", desc: "Fixed Sheriff clothing presets (thanks @Administries)" },
			4: { type: "add", desc: "Added ability to purchase boats (and boat garages)" },
			5: { type: "add", desc: "Added boat garage for SD and EMS" },
			6: { type: "add", desc: "Added flashlight attachment to SD pistol" },
			7: { type: "add", desc: "Added gunrack inventory instead of glovebox when opening SD vehicle glovebox" },
			8: { type: "add", desc: "Added binoculars to police armory" },
			9: { type: "add", desc: "Added car washes" },
			10: { type: "add", desc: "Added additional check to RC Cars to make sure you can't get out" },
			11: { type: "add", desc: "Added custom model for PD Pistol (Thanks @hnt-neptunium)" },
			12: { type: "bug", desc: "Fixed issue with vehicles stalling/breaking when being retrieved from garage" },
			13: { type: "bug", desc: "Fixed not being able to remove house keyholders from house phone app" },
			14: { type: "remove", desc: "Removed unused house transfer option" },
			15: { type: "change", desc: "Increased Mail delivery payout" },
			16: { type: "change", desc: "Increased Garbage payout" },
			17: { type: "change", desc: "Increased hunting payout" },
			18: { type: "change", desc: "Increased truck delivery payout" },
			19: { type: "add", desc: "Added another task to Big Smoke" },
			20: { type: "change", desc: "Tweaked Big Smoke rewards" },
			21: { type: "bug", desc: "Possibly fixed issue with jail time not saving correctly when logging out of server" },
			22: { type: "bug", desc: "Possibly fixed issue with items not being returned when released from jail" },
			23: { type: "bug", desc: "Fixed issue with fuel being reset when repairing vehicle at MRPD garage" },
			24: { type: "remove", desc: "Removed boat trailer from truck delivery job" },
			25: { type: "change", desc: "Updated server artifact" },
		},
	},
	...
	"Wednesday, December 1st 2021": {
		...
		"logs": {
			...
			2: { type: "add", desc: "Added Snowballs" },
			3: { type: "add", desc: "Added ability to spawn vehicles at it's last known location. (If your vehicle poofs, go to the garage app and press the car icon, it should respawn at it's last known location)" },
			4: { type: "change", desc: "Locating vehicles via the Garage app now works by sending you the vehicles last known location" },
			5: { type: "change", desc: "Drivers Licenses now show UI in chat instead of just text (They now show Citizen IDs as well)" },
			6: { type: "add", desc: "Added ability for Police to place Barriers and Cones" },
			7: { type: "add", desc: "The board in the MRPD classroom can now display images" },
			8: { type: "add", desc: "Added 10-13A (Panic) and 10-13B (Panic - Code 4)" },
			9: { type: "bug", desc: "Fixed off duty paychecks still being $10 instead of $25" },
			10: { type: "bug", desc: "Fixed some item description formatting" },
			11: { type: "change", desc: "Rewrote weather syncing" },
			12: { type: "bug", desc: "Weather now clears when entering Apartments/Houses (aka no more rain in apartments/houses)" },
			13: { type: "change", desc: "Meth process now gives more product" },
			14: { type: "change", desc: "Blackjack now has a max bet" },
			15: { type: "change", desc: "Decreased 0 and 00 payout in roulette" },
			16: { type: "add", desc: "Added DJ Booth to Arcade" },
			17: { type: "add", desc: "Del Perro Pier now has functional rides (Rollercoaster, Drop Tower and Ferris Wheel)" },
			18: { type: "add", desc: "Added cop4 animation" },
			19: { type: "change", desc: "Bennys can now be accessed from anywhere in the shop (also allows multiple people to access the same Bennys without messing with anyone)" },
			20: { type: "add", desc: "Added ability for Tuners to access extras menu" },
			21: { type: "remove", desc: "Removed ability for mechanics to access liveries in extras menu and /livery command." },
			22: { type: "bug", desc: "More possible fixes for vehicles not deleting when trying to store them" },
			23: { type: "bug", desc: "More possible fixes for vehicles not staying persistent when retrieving them from the garage" },
			24: { type: "add", desc: "Added Snap to Ground control UI" },
			25: { type: "bug", desc: "Fixed bug where if you leave the house while decorating and confirm the placement, it wipes all of your furniture." },
			26: { type: "remove", desc: "Removed laptop that would break decorating from furniture" },
			27: { type: "remove", desc: "Removed duplicate studio lights in furniture" },
			28: { type: "bug", desc: "Fixed issue where some furniture would get stuck when placing." },
			29: { type: "add", desc: "Added ability to unflip vehicles (via third eye)" },
			30: { type: "bug", desc: "Fixed being able to flip vehicles when pushing" },
			31: { type: "add", desc: "Added /911r [id] [message] to respond to emergency calls. This is available to Police, EMS, Mechanics and DOJ (the id is shown in the original emergency message)" },
			32: { type: "bug", desc: "Fixed Racing app persisting when switching characters" },
			33: { type: "change", desc: "Profile Pictures no longer load when in the conversation list. This should make conversations load instantly" },
			34: { type: "change", desc: "Increased Gold bar and uncut diamond prices" },
			35: { type: "change", desc: "Lowered loot chance of Diamonds and Gold from Mining" },
			36: { type: "change", desc: "Increased loot chance of Iron, Copper and Coal from Mining" },
			37: { type: "bug", desc: "Fixed more vehicle degradation errors" },
			38: { type: "bug", desc: "Fixed armor not saving/loading correctly" },
			39: { type: "change", desc: "Police and EMS now log in off duty" },
			40: { type: "bug", desc: "Fixed jail items not always being returned when being released from jail" },
			41: { type: "add", desc: "Added visual indicator for metal detecting" },
			42: { type: "change", desc: "Purging Nitrous no longer drains nitrous at a higher rate" },
			43: { type: "bug", desc: "MRPD Interior now refreshes automatically (this should hopefully fix some issues with not seeing the interior)" },
			44: { type: "change", desc: "Increased kit use distance" },
			45: { type: "bug", desc: "Fixed Pillbox Interior sometimes showing the default interior" },
			46: { type: "change", desc: "Rewrote speed limiter" },
			47: { type: "change", desc: "Tweaked weapon damages" },
			48: { type: "remove", desc: "Removed weird run after shooting" },
			49: { type: "add", desc: "Added deleting props when you hit them with your cars (props like signs, poles, etc.)" },
			50: { type: "bug", desc: "Fixed Hunting Rifle not using an animation when holstering/unholstering" },
			51: { type: "add", desc: "Added additional checks to make sure casino chips get updated when exchanging them" },
			52: { type: "bug", desc: "Fixed being able to press all arrows when resisting arrest and still passing" },
			53: { type: "change", desc: "Changed the player list format to show more players" },
			54: { type: "add", desc: "Added Manager rank to Mechanics" },
			55: { type: "add", desc: "Added sounds when opening Stash and Outfits in Houses and Apartments" },
			56: { type: "add", desc: "Added ability to view Performance Index via the OBD Scanner" },
